(function defineWeeklyVoting(root) {
    ...
    if (!constants || !data) {
        throw new Error("Stats V2 constants and data must load before weekly voting.");
    }
    ...
    function assertVoter(state, userId) {
        if (!constants.VOTER_IDS.includes(userId)
            || !state.settings.voters.some((voter) => voter.id === userId)) {
    ...
    function openCurrentIsoWeek(state, value = new Date(), timestamp = new Date().toISOString()) {
        ...
        if (existing) {
            if (existing.status === "CLOSED") {
                throw new WeeklyError("WEEK_CLOSED", `${descriptor.label} is already closed and cannot be reopened in Phase 4.`);
            }
        ...
        if (openWeek) {
            throw new WeeklyError("OPEN_WEEK_EXISTS", `Close ${openWeek.label} before opening ${descriptor.label}.`);
        }
    ...
    function findVote(state, weekId, participantId, userId) {
        return state.weeklyVotes.find((vote) => (
            vote.weekId === weekId && vote.participantId === participantId && vote.userId === userId
        )) || null;
    }

    function assertVoteInput(state, input) {
    ...
    function standoutCountForUser(state, weekId, userId, ignoredVoteId = null) {
        return state.weeklyVotes.filter((vote) => (
            vote.weekId === weekId
            && vote.userId === userId
            && vote.rating === "standout"
            && vote.id !== ignoredVoteId
        )).length;
    }

    function upsertVote(state, input, timestamp = new Date().toISOString()) {
        assertState(state);
        const normalized = assertVoteInput(state, input);
        const existing = findVote(state, input.weekId, input.participantId, input.userId);
        ...
        if (existing) {
            ...
            Object.assign(vote, {
                rating: input.rating,
                reasonTagIds: normalized.reasonTagIds,
    ...
    function removeVote(state, weekId, participantId, userId) {
        assertState(state);
        const week = findWeek(state, weekId);
        if (week.status !== "OPEN") throw new WeeklyError("WEEK_CLOSED", "Closed weeks are read-only.");
        findParticipant(state, participantId);
        assertVoter(state, userId);
        const existing = findVote(state, weekId, participantId, userId);
    ...
    function ratingScore(ratingId) {
        return ratingById.get(ratingId)?.score ?? null;
    }

    function deriveParticipantWeeklyMetrics(state, weekId, participantId) {
        findWeek(state, weekId);
        findParticipant(state, participantId);
        const votes = state.weeklyVotes.filter((vote) => vote.weekId === weekId && vote.participantId === participantId);
    ...
    function deriveWeeklyRanking(state, weekId) {
        findWeek(state, weekId);
        ...
        const items = votedParticipantIds.map((participantId) => ({
            ...
            metrics: deriveParticipantWeeklyMetrics(state, weekId, participantId),
            rank: null,
            tied: false
        }));
        items.sort((left, right) => (
            compareWeeklyMetrics(left.metrics, right.metrics)
    ...
    function createWeeklyPointsProvider(state, weekId) {
        findWeek(state, weekId);
        return (participant) => {
            const metrics = deriveParticipantWeeklyMetrics(state, weekId, participant.id);
    ...
    function filterWeeklyParticipants(state, weekId, userId, options = {}) {
        ...
        const historicalIds = new Set(state.weeklyVotes
            .filter((vote) => vote.weekId === weekId)
            .map((vote) => vote.participantId));
        ...
        return state.participants
            ...
            .filter((participant) => {
                const vote = findVote(state, weekId, participant.id, userId);
                if (evaluation === "all") return true;
                if (evaluation === "evaluated") return Boolean(vote);
                if (evaluation === "not-evaluated") return !vote;
                return vote?.rating === evaluation;
            })
    ...
    namespace.weekly = Object.freeze({
        ...
        removeVote,
        ratingScore,
        standoutCountForUser,
        deriveParticipantWeeklyMetrics,
        compareWeeklyMetrics,
        deriveWeeklyRanking,
        createWeeklyPointsProvider,
        weeklyProgress,
